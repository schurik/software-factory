...
- **A run that died part-way is picked up, not restarted.** `just resume <adw_id>` re-launches the same workflow against the same session with `--resume`: the agent phases already in the trace are answered from it (`↺ replayed`, no tokens, gates still checked against the tree as it is now), everything code owns runs again for real, and the chain reaches the phase that failed. Re-running a chain from the top pays for the whole thing twice.
