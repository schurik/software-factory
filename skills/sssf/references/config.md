...
**A run has three outcomes, and only two of them move the label.** Success flips to `done`, failure to `failed` — and a run that stopped at a **[human gate](#human-in-the-loop-gates)** exits `75` and is left on `running`, because it neither succeeded nor failed and a person has simply not answered yet. That is safe as a claim (only `queued` is dequeued, so nothing picks the issue up twice) but it does **not** come back on its own: `just approve` re-launches the run in a process the watcher never sees, so `running` outlives the run that earned it. `just pending` is where the truth lives until someone moves the label by hand. `pr_watch` has the mirror of this — a suspended review run is *not* marked `states.failed`, and the watcher skips a pull request whose session is waiting rather than paying to re-run the review every poll.
