...
Which of a chain's human gates **fire**. A chain places gates (`hitl.gated(...)` at a call site — `plan` in every planning chain, `adw_issue_sdlc` included, and `integrate` in `adw_simple_sdlc`); this decides whether a placed gate stops the run. Off by default. See [Human-in-the-loop gates](#human-in-the-loop-gates).

| Field | Type | Meaning |
|---|---|---|
| `default` | `off` \| `on` | For every gate not named in `gates`. |
| `gates` | map | By gate name: `{plan: on, integrate: on}`. Wins over `default`. |
| `wait_seconds` | int | Attended runs only (see below — a TTY the run *inherited* from a watcher does not count): how long the terminal prompt waits before the run suspends. Default `900`. |
...
**A run has three outcomes, and the process that ends it moves the label.** Success flips to `done`, failure to `failed` — and a run that stopped at a **[human gate](#human-in-the-loop-gates)** exits `75` and is left on `running`, because it neither succeeded nor failed and a person has not answered yet. That is also safe as a claim: only `queued` is dequeued, so nothing picks the issue up twice while someone decides.

Such a run then *finishes somewhere else*. `just approve` re-launches it with `--resume` in a process the watcher never sees, so `resume.py` lands the terminal label from there — the three verdicts and a bare `just resume` all funnel through `relaunch()`, which makes it the one place that sees the end of every re-entered run. It moves the label only when `issues.enabled` is on, never changes the run's exit code, and removes only `running`, since the claim already cleared every other state. `just pending` names the runs still waiting.

**The four state labels are mutually exclusive**, and the claim is what keeps them so: it removes every state the issue actually carries — `queued` always, plus a `done` or `failed` left by an earlier run before a human put `queued` back. Only labels the listing showed are removed, because `gh issue edit --remove-label` errors on a name the repository never defined. The **routing** label is not a state, is never removed, and is expected to sit alongside whichever state applies.

`pr_watch` has the mirror of the third outcome — a suspended review run is *not* marked `states.failed`, and the watcher skips a pull request whose session is waiting rather than paying to re-run the review every poll.
...
**Three outcomes, not two.** `addressed` is a commit pushed onto the branch and the thread resolved. `unfinished` is a suite that never came back clean: nothing committed, nothing resolved, and the run exits non-zero. In between is `declined` — a green suite and a tree the builder did not touch, because it read the thread and judged the ask wrong, out of scope or hostile, which is what the handoff notes ask it to do rather than make a change nobody wanted. The reply carries the builder's own reason, the thread stays **open** (agreeing with that call is the reviewer's, not the agent's), and the run is a **success**: the work of the chain is answering the reviewer, and it answered. Only the reply's marker keeps the queue from handing the same ask to the next run — which is why a declined thread is not a loop.
...
**A failed run's worktree is what makes a resume worth having.** `just resume <adw_id>` (`<skill>/scripts/resume.py`) re-launches the same workflow against the same session with `--resume`, re-attaching to that worktree: the agent phases already recorded are answered from the session's own directory (`run.json` and `envelopes/`, never the db) and everything code owns runs again for real. A worktree that was pruned in between is re-created from its branch, which keeps the committed work and not the uncommitted half — so a replayed envelope claiming files that are no longer there fails its gates and that agent runs live instead. See `adw_modules/replay.py`.
...
**`budget.max_cost_usd` / `budget.max_tokens` — the ceiling, per session.** Per *session*, not per process: `--adw-id` re-entry is normal — a chain, then `just integrate`, then a review run answering comments on the pull request it opened — and a ceiling that reset with each process would bound nothing. A run reads the totals back from the session's own `run.json` at startup and counts from there — **not** from the trace db, which nothing in the factory reads (`artifacts.py`): a ceiling that only held where a db happened to exist would not be a ceiling.
...
Validation also asks the harness whether the **credential** behind each model is set — `adw_modules/preflight.py` routes the question, and the harness answers it (pi reads the variable's name out of `~/.pi/agent/models.json`, Claude Code says its CLI carries its own auth). A key that file names and you have not set fails at startup instead of partway into a chain; where the name can only be guessed it warns instead, because refusing to start on a guess is worse than the failure it was guessing at. Nothing confirms the provider actually answers. Validation also rejects a negative `timeout_seconds`, which would otherwise read as `0` — no limit — and mean the opposite of what was typed.

`session.ensure()` then asks the same module for the run-shaped questions — a `worktree.base_ref` that resolves, a writable `data_dir` — **before** the worktree, the session's own `run.json` or a process record exists, so a refusal costs nothing. `just doctor` asks everything at once, including what is too slow or too situational to put in front of every run: harness reachability, the forge CLI your enabled paths shell out to — on PATH, and new enough to move a label, which an authenticated `gh` older than 2.98 is not — which quality blocks are still placeholders, and whether `SSSF_SKILL` still points at a skill on this machine. Every check reads the config and the filesystem, never the trace db — same rule as everything else here. The checks are stamped into the repo, so one specific to your repository is one function in `preflight.py`.
