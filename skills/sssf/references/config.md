...
`session.ensure()` then asks the same module for the run-shaped questions — a `worktree.base_ref` that resolves, a writable `data_dir` — **before** the worktree, the session's own `run.json` or a process record exists, so a refusal costs nothing. `just doctor` asks everything at once, including what is too slow or too situational to put in front of every run: harness reachability, the forge CLI your enabled paths shell out to, which quality blocks are still placeholders, and whether `SSSF_SKILL` still points at a skill on this machine. Every check reads the config and the filesystem, never the trace db — same rule as everything else here. The checks are stamped into the repo, so one specific to your repository is one function in `preflight.py`.
