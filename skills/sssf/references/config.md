...
Validation also asks the harness whether the **credential** behind each model is set — `adw_modules/preflight.py` routes the question, and the harness answers it (pi reads the variable's name out of `~/.pi/agent/models.json`, Claude Code says its CLI carries its own auth). A key that file names and you have not set fails at startup instead of partway into a chain; where the name can only be guessed it warns instead, because refusing to start on a guess is worse than the failure it was guessing at. Nothing confirms the provider actually answers.

`session.ensure()` then asks the same module for the run-shaped questions — a `worktree.base_ref` that resolves, a writable `data_dir` — **before** a session row, a branch or a process record exists, so a refusal costs nothing. `just doctor` asks everything at once, including what is too slow or too situational to put in front of every run: harness reachability, the forge CLI your enabled paths shell out to, which quality blocks are still placeholders, and whether `SSSF_SKILL` still points at a skill on this machine. The checks are stamped into the repo — a check specific to your repository is one function in `preflight.py`.
