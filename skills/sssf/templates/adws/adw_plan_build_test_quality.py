...
Usage:
    uv run adws/adw_plan_build_test_quality.py "<prompt or path/to/prompt.md>" [--config adws/adw_sssf_config/sssf.config.yaml] [--adw-id a1b2c3d4] [--resume] [--hitl all|none|every|plan]

Phases: engineer(request) -> planner [-> engineer(approve_plan) ...] -> builder -> [code(verify) -> code(test) -> builder(fix)] bounded -> git(commit)
...
from adw_modules import (agents, gates, git_helper, hitl, integration, quality,
                         session, utils)
