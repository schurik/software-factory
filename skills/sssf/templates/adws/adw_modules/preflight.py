...
def quality(main_root: Path) -> list[Finding]:
    ...
    try:
        ...
        return [Finding(check="quality", level="warn",
                        detail=f"could not inspect adws/adw_modules/quality.py: {error}",
    ...
    return [Finding(
        ...
        fix="replace the `_placeholder(...)` argv in adws/adw_modules/quality.py with "
...
def skill() -> list[Finding]:
    """Whether SSSF_SKILL still points at a skill directory.
    ...
    if not (root / "scripts" / "up.py").is_file():
        return [Finding(
            ...
            fix="set SSSF_SKILL in .env to this machine's skill directory, or "
                "re-run install.py from the target repo root")]
    return [Finding(check="SSSF_SKILL", detail=str(root))]
