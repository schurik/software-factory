...
- `retries=N` on an **agent** phase = extra gate-correction rounds re-sent into the same session, with its context intact on any harness. Code-phase re-execution is not implemented in v1.
