...
and annoying to discover later — a silently-passing test suite, a machine moving
your main branch, who may start a run at all. **Put these to the engineer before
you run the installer**, in one message, with the defaults named so they can say
"all defaults" and be done.

The first has no default: **which harness**. It decides which roster, which
prompts, which extensions and which `.env.sample` get stamped, so the installer
asks it rather than guessing — pass `--harness <name>` to answer ahead of time,
and without a terminal to ask at, a missing flag is an error rather than a silent
`pi`.
...
Leave `--harness` off and it asks, listing what this skill ships. Either way
nothing is written until the question is answered, so an abort leaves the repo
untouched.
...
|---|---|---|
| `adws/adw_sssf_config/sssf.config.yaml` | assembled: `templates/harnesses/<harness>/defaults.yaml` + `templates/config/base.yaml` + `templates/harnesses/<harness>/agents.yaml` | yes — the agent roster |
...
**Both are harness-specific, which is why the harness is asked first.** On `pi`, `harness_engineering/` ships `subagents.ts` — the extension backing `subagent_create` / `_continue` / `_list` / `_remove`, wired to the planner and scout in the roster and described in their system prompts. On `claude_code` those tools do not exist, so that section is absent from the prompts and the directory ships a README explaining what `mcp:` / `agents:` / `plugin:` entries mean instead.

## Idempotency

Re-running is safe. `install.py` skips **every** file that already exists — your config, your prompts, and previously stamped code alike — and reports what it skipped, so a second run doubles as a drift check. It still asks which harness, because the answer decides what it would stamp into the gaps; give it the same one the repo already runs, or `--harness` to skip the question. To refresh stamped code (`adw_modules/`, the starter `adw_*.py`) to the skill's current version, run with `--force` — but know that `--force` overwrites ALL existing stamped files, including `sssf.config.yaml` and `prompt_engineering/`, so commit or back up user-owned edits first.
...
Green means the whole path works: config validated, session minted, the harness ran, envelope parsed, events landed in `adws/adw_data/sssf.db`. Verify the trace exists before trusting anything larger:
