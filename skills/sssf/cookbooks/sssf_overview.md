...
│   ├── branches.py              the run's branch NAME — the only place it's built or parsed  ·  plan() names it and links it to its issue at the forge
...
**Nothing polls unless something is polling.** The three long-running parts — the trace UI, the issue watcher, the review watcher — start together with `just up`, one foreground process that owns all three and stops them together. This matters because the failure mode is silent: a labelled issue with no watcher running looks exactly like a labelled issue with a busy factory. `just status` (and the badges in the trace UI's top bar) reads the watcher heartbeats out of the trace db and probes their pids, so "is it up" has an answer. Never tell the engineer a run will be picked up without checking one of the two.
