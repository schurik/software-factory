...
| `artifacts.py` | the session directory as a READABLE record — `run.json` (which workflow, what argv, which pid, how it ended) and `envelopes/<phase_id>.json` (one per agent phase, where the per-agent `envelope.json` is last-wins). Everything the factory needs to know about a session it has already run comes from here, never from sqlite: a run works with the db deleted |
...
| `tracer.py` | append JSONL **and** insert every event into `sssf.db` as it happens. A RUN ONLY WRITES: the db is the visualizer's mirror, and its module-level readers serve the maintenance tools (the watchers, `just worktrees`), never a workflow |
