...
- **One entry point.** `uv run asf/asf.py run <workflow> "<prompt>"`. `list` and
  `check` beside it.
...
2. Run `uv run asf/asf.py list` and print it — one line per workflow, name and
   description — and **wait for the engineer's request.**
...
| Request | Do |
|---|---|
| install / set up the factory here | `uv run <skill>/scripts/install.py --harness claude_code\|pi`, then `uv run asf/asf.py check` |
| run a workflow | `uv run asf/asf.py run <name> "<prompt or path>" [--hitl all\|none\|plan]` |
...
| tailor an agent's TASK for one workflow | add `asf/workflows/<name>/tasks/<key>.md` — keys are the stage's TASKS (plan, build, fix). Keep the `## Report` block matching the type; `check` verifies it |
...
| pick a failed run back up | re-run with `--adw-id <id> --resume` |
| a run is waiting at a gate | read `asf/data/sessions/<id>/run.json` `waiting_for`; answering needs `engine.hitl.answer(...)` until the gate CLI lands |
...
This is slice one: plan, build, verify, commit; two workflows; the loader and
runner; the installer. Not yet ported from sssf: review, document, integrate
stages; issue and pull-request inputs; the gate-answering CLI; doctor; the
justfile. The visualizer needs no port — same db, same schema.
