...
`base_commit` is passed explicitly for one specific reason. On the branch-exists path it is otherwise `merge_base(path, base_ref, "HEAD")`, which for a linked branch answers with the *local* base tip while the branch was actually cut from **origin's**. `base_commit` is not automatically what every diff in the run measures from — `changes.resolve_base()` takes a *ref* and answers `merge_base(ref, HEAD)`, so a chain has to hand it the commit, not the ref, to get the honest answer. The sdlc chains do this safely by pinning their own `baseline = git_helper.rev(run.repo_root, "HEAD")` right after the session opens; `adw_document.py` prefers `base_commit` over `base_ref` for the same reason (`base = base or run.workspace.base_commit or run.workspace.base_ref or "main"`). `integration.py` is the one consumer that reads it directly, comparing HEAD against it to decide a branch has nothing to land. So the honest claim is narrower than "every diff measures from it": `base_commit` is what `integration.py` compares against, and it is the correct branch point for any chain that is careful to measure from the commit rather than the ref — a stale `base_ref`-derived answer would otherwise quietly widen a linked branch's diffs by whatever origin was ahead by.
...
Built as designed. `branches.py` owns the format, `issues.py` grew `peek()` and `develop()`, `session.ensure()` composes them, `worktree.py` stayed forge-free, and the plan's notes ride the workspace line — the five design sections all landed. Five things came out differently:

- **Three functions the design assumed could not fail, could — and all three were found by review, not by writing.** Each traced back to the same root: code copied from `fetch()`, which raises *by design* — a chain that cannot read its issue has nothing to plan against — into a function that must not raise, without the reasoning coming along. `peek()` parsed JSON that could be valid but not an object (`json.loads("null")` returns `None`, and `.get` on `None` raises), and coerced a `number` field that need not be numeric. `_base_ref_of()` called `git rev-parse --abbrev-ref HEAD`, which exits 128 on a repository with no commits yet — a state `has_remote` does not rule out, and one `worktree.ensure()` already guarded for elsewhere. `_run()` itself raised `FileNotFoundError` when the configured forge CLI was not on PATH, which meant the phase failed its own Verification bullet — a run with a bogus `develop_command` has to finish accepted, not crash the interpreter. Fixed once, at `_run`, which repaired `comment()` and `set_state()` at the same time. All three mattered more than a one-line fix suggests, because `peek()` runs **before the Tracer exists**: a raise there is a bare traceback with no trace row, no session row, and no console line saying why.

- **The recompute bug had a second door.** The design's fix — read the recorded branch before recomputing — assumed that record would be there. It is not always: the metadata file is gitignored and local-only, so a fresh clone, a `git clean -fdx`, or a second checkout of the same repository leaves a joining run holding only an adw_id, with no record to read. It would fall back to the bare `<prefix><adw_id>`, find no such branch, and cut a second one — worst in `adw_pr_review`, which has no integration phase, so the orphaned commits would never reach the pull request. `plan()` now asks git for an existing `<prefix><adw_id>*` branch and adopts it when exactly one matches; ambiguity falls through to inventing a name rather than guessing.

- **Nothing validated the name the forge handed back.** `develop()` deliberately reads the branch name out of `gh`'s printed tree URL rather than trusting the request, because the forge sanitises names — but the design never checked the result still decoded to this adw_id, and `adw_pr_review` refuses to run on a branch it cannot decode. `plan()` now checks `session_of(linked.branch, prefix) == adw_id` before adopting the forge's name; a name that does not decode falls back to treating the branch as created-but-unusable rather than adopted.

- **The `base_commit` rationale itself needed correcting, not only the code around it.** The design's §4 originally claimed "every diff in the run measures from `base_commit`," which is not true: `changes.resolve_base()` takes a *ref* and answers `merge_base(ref, HEAD)`, and `workspace.base_commit` has exactly one consumer outside the tracer — `integration.py`, comparing it against HEAD to decide a branch has nothing to land. That claim is corrected above and in `data_types.py`'s `BranchPlan` docstring. Separately, and for real: `adw_document.py` *was* the one chain actually measuring from the wrong base — it fell back to `base_ref`, which on a linked branch resolves older than the real branch point — and it now prefers the pinned `base_commit` for the reason described above.

- **This phase brought the repository its first test suite.** `skills/sssf/tests/` — 53 test cases across four files — sits outside `templates/`, so `install.py` cannot stamp it into a consuming repo, and its `conftest.py` puts the real template tree on `sys.path` so the tests exercise the shipped files rather than a copy.

Still outstanding, and worth saying plainly: no end-to-end verification against a real GitHub forge has run yet. Every claim about `gh issue develop`'s stdout shape, its `--base` semantics, and its idempotence on a rerun rests on mocks. That step is the engineer's to run.
