...
| 9 | [Human-in-the-loop gates](phase-9-hitl-gates.md) | A run stops after any phase and hands its artifact to a human, who approves, rejects with notes (the agent revises in the same session), or aborts; per gate and per run, a policy trusts or forces it | **brainstorm** — options and open questions, not yet planned |
...
Phase 2 (worktree) ─────┤
                        └─→ Phase 5 (issue tracking) ─┬─→ Phase 7 (one-command startup)
                                                      └─→ Phase 8 (linked branches)
Phase 2 (worktree) ─→ resume/replay ─→ Phase 9 (HITL gates)
```
...
- **Phase 8 depends on Phases 2 and 5**, both built. It is the smallest item on the list and touches only the naming and creation of the run's branch — but it is the one that makes an in-flight run visible from the issue that asked for it, which until now was answerable only after the pull request appeared.
- **Phase 9 depends on Phase 2 and on `--resume`**, both built: a gate that suspends a run is a resume with one more file to read, and the artifact the human reviews lives in the run's own worktree. It is independent of Phases 3 to 8; the forge channel it sketches reuses Phase 5's issue and pull-request modules but does not need them for a first slice.
