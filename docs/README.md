...
Upstream SSSF is a Claude Code plugin (`sssf`, in `plugins/sssf/`) whose skill stamps a deterministic Python control plane into any repository. Its core decision — **code owns sequencing, retries and acceptance; the agent owns only the work inside one bounded phase** — is the right one, and it is the expensive part to get right. It is already built:
...
| Live trace UI | `plugins/sssf/skills/sssf/apps/visualizer/` (Bun + Vue) |
...
The ten hard rules in `plugins/sssf/skills/sssf/SKILL.md` apply to everything generated here. The ones that bite most often:
